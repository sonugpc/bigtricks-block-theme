<?php
/**
 * Main index template (fallback for all archives/home)
 *
 * @package Bigtricks
 */

// Redirect to front-page for home
if ( is_home() && is_front_page() ) {
	// front-page.php will handle this
}
?>
<?php get_header(); ?>
...
<main class="max-w-[1400px] mx-auto px-4 py-6 md:py-8 flex flex-col lg:flex-row gap-8 flex-1 w-full" id="main-content">
	<div class="flex-1 min-w-0">
		...
		<h1 class="text-2xl font-black text-slate-900 flex items-center gap-3 mb-6">
			...
			if ( is_archive() ) {
				...
				printf( esc_html__( 'Search Results: "%s"', 'bigtricks' ), esc_html( get_search_query() ) );
			} else {
				esc_html_e( 'Latest Updates', 'bigtricks' );
			}
			?>
		</h1>
			...
			<?php if ( have_posts() ) : ?>
				...
				<article class="bg-white rounded-3xl shadow-sm border border-slate-200/60 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group flex flex-col h-full">
					<div class="flex flex-col sm:flex-row h-full">
						<a href="<?php the_permalink(); ?>" class="sm:w-[280px] shrink-0 bg-slate-50 p-6 flex items-center justify-center sm:border-r border-b sm:border-b-0 border-slate-100 relative" tabindex="-1" aria-hidden="true">
							<img src="<?php echo esc_url( $thumb_url ); ?>" alt="<?php the_title_attribute(); ?>" class="max-h-[180px] max-w-full object-contain mix-blend-multiply transition-transform duration-500 group-hover:scale-105" loading="lazy" decoding="async">
							<div class="absolute top-3 left-3">
								<?php echo wp_kses_post( bigtricks_deal_type_badge( $post_id ) ); ?>
							</div>
						</a>
						<div class="p-5 sm:p-6 lg:p-8 flex-1 flex flex-col justify-between min-w-0">
							...
							<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-4 border-t border-slate-100">
								...
								<div><?php echo wp_kses_post( bigtricks_deal_cta_button( $post_id ) ); ?></div>
							</div>
						</div>
					</div>
				</article>
				...
				<?php the_posts_pagination( [
					'prev_text' => esc_html__( 'Previous', 'bigtricks' ),
					'next_text' => esc_html__( 'Next', 'bigtricks' ),
				] ); ?>
